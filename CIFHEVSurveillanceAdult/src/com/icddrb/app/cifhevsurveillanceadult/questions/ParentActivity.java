package com.icddrb.app.cifhevsurveillanceadult.questions;
...
import com.icddrb.app.cifhevsurveillanceadult.BaseActivity;
import com.icddrb.app.cifhevsurveillanceadult.CommonStaticClass;
import com.icddrb.app.cifhevsurveillanceadult.Options;
import com.icddrb.app.cifhevsurveillanceadult.adapters.SpinAdapter;
import com.icddrb.app.cifhevsurveillanceadult.db.DatabaseHelper;
import com.icddrb.app.cifhevsurveillanceadult.R;
...
public class ParentActivity extends BaseActivity implements FormListener {
	...
	private void loadGuiFrmHHID(ViewGroup v) {
		...
		confButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (((Spinner) vg.findViewById(R.id.sphosid))
						.getSelectedItemPosition() == 0) {
					CommonStaticClass.showMyAlert(con, "Message",
							"Please fill all fields correctly");
					return;
				}
				if (((Spinner) vg.findViewById(R.id.sppid))
						.getSelectedItemPosition() == 0) {
					CommonStaticClass.showMyAlert(con, "Message",
							"Please fill all fields correctly");
					return;
				}
				if (((EditText) vg.findViewById(R.id.txtid)).getText()
						.toString().length() != 3) {
					CommonStaticClass.showMyAlert(con, "Message",
							"Please fill all fields correctly");
					return;
				}
				moID = ((EditText) vg.findViewById(R.id.txtid)).getText()
						.toString();
				String moIDre = ((EditText) vg.findViewById(R.id.txtidre))
						.getText().toString();
				hhID = ((Spinner) vg.findViewById(R.id.sphosid))
						.getSelectedItem().toString();
				ppID = ((Spinner) vg.findViewById(R.id.sppid))
						.getSelectedItem().toString();
				String hhIDre = ((Spinner) vg.findViewById(R.id.sphosidre))
						.getSelectedItem().toString();
				String ppIDre = ((Spinner) vg.findViewById(R.id.sppidre))
						.getSelectedItem().toString();
				if (!(moID + hhID + ppID)
						.equalsIgnoreCase((moIDre + hhIDre + ppIDre))) {
					CommonStaticClass.showMyAlert(con, "Message",
							"Please fill all fields correctly");
					return;
				}
				CommonStaticClass.dataid_store = "";
				CommonStaticClass.HouseholdCode = "";
				CommonStaticClass.dataId = hhID + ppID + moID;
					...
					new Thread() {
						...
						public void run() {
							try {
								...
								CommonStaticClass.dataid_store = CommonStaticClass.dataId;
							} catch (Exception lg) {
								progressDialog.dismiss();
								CommonStaticClass.showFinalAlert(con,
										"Ensure your Asset ID");
	...
	private void updateTableDataFrmHHID(ViewGroup v) {
		...
		try {
			/*
			 * if (Code_validation(v) == false) {
			 * CommonStaticClass.showMyAlert(thisactivity, "Error",
			 * "Participant code is not valid for this Upazilla!!!"); return; }
			 */
			String sql = "Select * from  "
					+ CommonStaticClass
			...
			if (cursor.getCount() != 0) {
				 ...
				 * "This id is already exist!"); return;
				 */
				...
				/*
				 * if (CommonStaticClass.dataId.length() < 8) {
				 * CommonStaticClass.showMyAlert(con, "Error",
				 * "Select all fields"); return; }
				 */
				String entryDate = CommonStaticClass.GetDate().toString();

				sql = String
						.format("Insert into %s (dataid,hospitalID,VersionNo,assetid, EntryBy,EntryDate) VALUES('%s','%s','%s','%s','%s','%s')",
								CommonStaticClass
										.GetTableName(CommonStaticClass.questionMap
												.get(CommonStaticClass.currentSLNo)
												.getQvar()),
								CommonStaticClass.dataId, hhID,
			...
			if (dbHelper.executeDMLQuery(sql)) {
				...
				if (!gotoskip()) {
					if (qName.equalsIgnoreCase("q3_4")) {
						if (op.qidList.get(0).equalsIgnoreCase("q3_4_5")
								&& aaa.get(0) == 1) {
							nullifyWithInRange(CommonStaticClass.questionMap
									.get(CommonStaticClass.currentSLNo)
									.getQvar(), "q41m");
							CommonStaticClass.findOutNextSLNo(qName, "q41m");
							CommonStaticClass.nextQuestion(ParentActivity.this);
						} else {
							nullifyWithInRange(CommonStaticClass.questionMap
									.get(CommonStaticClass.currentSLNo)
									.getQvar(), CommonStaticClass.questionMap
									.get(CommonStaticClass.currentSLNo)
									.getQnext1());
							CommonStaticClass.findOutNextSLNo(
									qName,
			...
			public void afterTextChanged(Editable s) {
				...
				if (qName.equalsIgnoreCase("q3_3")) {
					...
					if (lineNumber.length() > 5) {
						CommonStaticClass.showMyAlert(con, "Message",
								"Number should be in 5 digit");
						if (lineNumber.length() > 5) {
							infoText.setText(lineNumber.substring(0,
									lineNumber.length() - 1));
						}
						// infoText.setText("");
						return;
